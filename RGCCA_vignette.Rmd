...
It implies that whatever the mode of deflation, the block components can always 
be expressed in terms of the original data matrix.
...
Equations (\ref{AVE_X}) is defined for a specific block component. The cumulative AVE is 
obtained by summing these individual AVEs over the different components. 
However, this sum applies only for orthogonal block components.  For correlated 
components, we follow the QR-orthogonalization procedure described in \cite{Zou2006} 
to take into account only the increase of AVE due to the addition of the new 
components. 
...
the maximum values of the range to be tested can be given. The set of parameters 
...
the fitted `rgcca_stability` object contains can be visualized using 
`print()/plot()`function and return the same graphical representations as for RGCCA.
...
We use here the `n_mark` parameter to display the top 50 variables of GE.
