...
highly multivariate datasets constitute critical issues for effective analysis 
leading to valuable knowledge. For the statistical computing environment 
R \cite{R2022}, various R packages are available for multiblock data analysis.
Four mainstream references are $\small{\texttt{mixOmics}}$ \cite{Rohart2017}, 
$\small{\texttt{multiblock}}$ \cite{Liland2022}, $\small{\texttt{ade4}}$ 
(\cite{Dray2007}), and $\small{\texttt{FactoMineR}}$ \cite{Le2008}. Each 
package uses its way of specifying multiblock models and storing the results.
Regularized Generalized Canonical Correlation Analysis is a unified statistical 
framework for an astonishinggly large number of multiblock component methods. 
The RGCCA package (Girka et al, 2023) implements this framework, and its 
usefulness and versatility are illustrated in this paper.
...
function for specifying the multiblock models. It also provides several plots 
for diagnostics or visualization of the results from multiblock analysis.
Package $\small{\texttt{RGCCA}}$  is available from the 
...
Section 2 presents the optimization problem under which all the algorithms 
proposed in the RGCCA framework were designed. The theoretical foundations of 
the RGCCA framework \cite{Tenenhaus2011, Tenenhaus2014, Tenenhaus2015, 
Tenenhaus2017}, are briefly summarized in Section 3, while Section 4 and 5 
illustrates the use of package RGCCA with code examples. The final Section 6 
concludes the paper.
