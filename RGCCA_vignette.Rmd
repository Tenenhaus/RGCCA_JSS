...
(`ade4::mfa()/FactoMineR::MFA`). Note that, in this case, block components do not 
belong to their block space and are correlated. On the contrary, we follow the 
deflation strategy described in \cite{Chessel1996} (`ade4::mcoa()`) for Multiple 
...
are available as output of the `rgcca()` function in `fit$a` and correspond exactly to the weight vectors reported in Figure 5 of \cite{Tenenhaus2011}. 
...
biplot (`type = "biplot"`).  This graphical display allows visualizing the sources of variability within blocks, 
...
Interestingly, the `print()` function reports the arguments
...
p-value. This procedure is available through the `rgcca_permutation()` function.
...
The fitted permutation object, `perm_out`, can be directly provided as the output 
...
predicting the location. The argument `response=3` of the `rgcca()` function 
encodes this design. 
...
classifier (`prediction_model = "lda"`). Also, we decide to upper bound the sparsity 
...
`rgcca_cv()` relies on the `caret` package. As a direct consequence, an astonishingly 
...
the usual `print()` and `plot()` functions.
...
We use the `n_mark` parameter to display the top 50 variables of GE.
