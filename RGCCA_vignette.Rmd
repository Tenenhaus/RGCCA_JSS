...
author:
  - name: Fabien Girka
    affiliation: |
      ...
      | CentraleSupelec
    affiliation2: Paris Brain Institute  
  ...
  - name: Caroline Peltier
    affiliation: |
      | INRAE
      | Paris Brain Institute \AND
  - name: Arnaud Gloaguen
    affiliation: |
      | CEA 
  - name: Vincent Guillemot
    affiliation: 'Pasteur \AND'
  ...
  - name: Arthur Tenenhaus
    ...
    affiliation: |
      ...
      | Université Paris-Saclay
      | CentraleSupelec
    email: \email{arthur.tenenhause@centralesupelec.fr}
...
abstract: 
    Multiblock component methods concern the analysis of several sets of variables 
    (blocks) observed on the same group of individuals. The Regularized Generalized
    Canonical Correlation Analysis (RGCCA) framework gathers fifty years of 
    multiblock component methods and offers a unified implementation strategy 
    for these methods. The RGCCA package implement this framework. 
    The software also produces graphical ouptuts and statistics to assess the 
    robustness/significance of the analysis. The RGCCA package is freely available 
    on the ComprehensiveR Archive Network (CRAN) http://www.r-project.org/ and on 
...
such as "learning from multimodal data", "data integration", "multiview data", 
"multisource data", "data fusion" or "multiblock data analysis". Appropriate 
statistical methods and dedicated software able to cope with these multiple 
...
most algorithms proposed in the RGCCA framework were designed. The RGCCA 
framework gathers several algorithms already presented in 
...
$f(\ma a_1, \ldots,\ma a_J):\mathbb{R}^{p_1}\times \ldots \times \mathbb{R}^{p_J} \xrightarrow{}\mathbb{R}$ (i.e. for each $j$, $f$ is a convex function of 
...
$\ma a = \left( \ma a_1^\top, \ldots, \ma a_J^\top\right)^\top$ 
...
In the multiblock data analysis literature, all blocks $\X_j ,j = 1,\ldots,J$ 
...
$\X_j$ by $\X_j^{(1)}$ its projection onto either: (i) the orthogonal subspace 
of $\ma y_j^{(1)}$ if orthogonal components are desired: 
$\X_j^{(1)} = \X_j - \ma y_j^{(1)} \left( {\ma y_j^{(1)}}^\top \ma y_j^{(1)} \right)^{-1}{\ma y_j^{(1)}}^\top \X_j$, 
or (ii) the orthogonal subspace of $\ma a_j^{(1)}$ for orthogonal weight vectors 
$\X_j^{(1)} = \X_j - \X_j\ma a_j^{(1)} \left( {\ma a_j^{(1)}}^\top \ma a_j^{(1)} \right)^{-1}{\ma {a}_j^{(1)}}^\top$.
...
$\ma y_j^{(2)} = \X_j^{(1)}\ma a_j^{(2)} = \X_j \ma a_j^{(2)}$ 
...
Therefore any block component $\y_j^{(2)}$ belonging to the range space of 
$\X_j^{(1)}$ can also be expressed in terms of the original block $\X_j$: that 
is, it exists $\ma a_j^{(2)^\star}$ such that $\y_j^{(2)} = \X_j^{(1)}\ma a_j^{(2)} = \X_j\ma a_j^{(2)^\star}$. 
...
$$\X_{J+1}^{(1)} = \left(\mathbf{I} - \ma y_{J+1}^{(1)} \left( {\ma y_{J+1}^{(1)}}^\top \ma y_{J+1}^{(1)} \right)^{-1}{\ma y_{J+1}^{(1)}}^\top \right) \X_{J+1} = \left[ \X_1^{(1)}, \ldots, \X_J^{(1)} \right].$$
...
The best set of tuning parameters is then the set that yields the smallest
p-value. This procedure is available through the $\small{\texttt{rgcca\_permutation}}$ function.
...
By default, the $\small{\texttt{rgcca\_permutation}}$ function generates 10 sets of tuning 
...
The fitted permutation object, $\small{\texttt{perm\_out}}$, can be directly provided as the output 
of $\small{\texttt{rgcca()}}$ and visualized/bootstrapped as usual.
...
$\small{\texttt{rgcca\_cv()}}$ relies on the $\small{\texttt{caret}}$ package. As a direct consequence, an astonishingly 
large number of models are made available (see $\small{\texttt{caret::modelLookup()}}$). Results 
...
individuals, the function $\small{\texttt{rgcca\_transform}}$ is available.
...
of the literature, including $\small{\texttt{ade4}}$ and $\small{\texttt{factomineR}}$.
\end{itemize}
...
multiway data \citep{Gloaguen2020, Girka2023}, RGCCA for (sparse and
irregular) functional data \citep{Sort2023}. In addition, maximizing
successive criteria may be seen as suboptimal from an optimization point
