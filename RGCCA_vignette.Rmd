...
the RGCCA package. The Russett dataset [@Russett1964] is studied in [@Gifi1990]. 
...
blocks <- list(
  ...
  lab = as.matrix(factor(
    apply(Russett[, 9:11], 1, which.max),
    labels = c("Stable", "Unstable", "Dictator")
  ))
)
...
training <- lapply(blocks, function(x) x[inTraining, , drop = FALSE])
testing  <- lapply(blocks, function(x) x[-inTraining, , drop = FALSE])
