...
framework gathers several algorithm that has already been presented in 
[@Tenenhaus2011, @Tenenhaus2014, @Tenenhaus2015, @Tenenhaus2017]. It is recalled 
...
of size $p = \sum_{j=1}^{J}p_j$. Moreover, the vertical concatenation of 
column vectors is denoted $\ma a = \left( \ma a_1; \ldots; \ma a_J \right)$ 
...
in [@DeLeeuw1994}] and is called Block Relaxation or cyclic Block Coordinate 
Ascent (BCA). 
...
properties is given @Tenenhaus2017.


# The RGCCA framework 

The theoretical foundations of the Regularized Generalized Canonical Correlation 
Analysis (RGCCA) framework - that were previously published [@Tenenhaus2011 ; @Tenenhaus2014 ; @Tenenhaus2015 ; @Tenenhaus2017] - are briefly 
summarized.
...
column vector. A composite variable $\eta_j$ is defined as the linear combination 
of the elements of $\boldsymbol x_j$: $\eta_j = \ma a_j^\top \boldsymbol x_j$.
Therefore the covariance between two composite variables is 
$\mathbf{a}_j^\top \ma \Sigma_{jk} \mathbf{a}_k$. The RGCCA framework aims at 
extracting the information which is shared by the $J$ random composite variables 
...
"connected" composites $\eta_j$ and $\eta_k$ subject to specific constraints on 
...
Several instantiations of the RGCCA framework were proposed in [@Tenenhaus2011, 
...
$\mathbf{M}_j$ is positive definite matrix of order $p_j$. The optimization 
...
represents a set of $p_j$ variables observed on $n$ individuals. The number and 
the nature of the variables may differ from one block to another, but the 
individuals must be the same across blocks. We assume that all variables are 
centered. The most recent formulation of RGCCA [@Tenenhaus2017] subsumes fifty 
years of multiblock component methods. It provides improvements to the initial 
version of RGCCA [@Tenenhaus2011] and is defined as the following optimization 
problem:
...
plug-in approach leads to the RGCCA optimization problem [@Tenenhaus2011]. 
It is worth pointing out that for each block $j$, an appropriate shrinkage 
parameter $\tau_j$ can be obtained using various analytical formulae (see for 
instance [@Ledoit2004, @Schafer2005, @Chen2011]). As $\mathbf{M}_j$ must be 
positive definite, $\tau_j = 0$ can only be selected for a full rank data matrix 
$\mathbf{X}_j$.
...
the correlation. Setting the $\tau_j$ to 0 will force the block components to 
...
by [@Schafer2005], or guiding by the context of application by cross-validation 
or permutation. 
...
[@Hotelling1936] Interbattery Factor Analysis [@Tucker1958] and Redundancy 
Analysis [@Wollenberg1977] - to three or more sets of variables. The second one 
relates to the ability of taking into account some hypotheses on between-block 
...
capture both correlation or covariance-based criteria. The fourth one relates 
to the function $g$ that enables to consider different functions of the 
...
be handled. This triplet of parameters offers a flexibility to RGCCA and allows 
to encompass a large number of multiblock component methods that were published 
...
multiblock component methods. For a complete overview see [@Tenenhaus2017]. 
...
Canonical correlation analysis [@Hotelling1936] is the seminal paper for the 
first family and Tucker's inter-battery factor analysis [@Tucker1958] for 
...
Correlation Analysis [@Vinod1976; @Leurgans1993; @Shawe2004]. For various 
...
| **Interbattery Factor Analysis** [@Tucker1958] or **PLS Regression** [@Wold1983]      | $x$    | $\tau_1 = \tau_2 = 1$  | $\mathbf{C}_1$|
| **Redundancy Analysis** [@Wollenberg1977]           | $x$    | $\tau_1 = 1$ ; $\tau_2 = 0$ | $\mathbf{C}_1$|
| **Regularized Redundancy Analysis** [@Takane2007; @Bougeard2008 ; @Qannari2005]           | $x$    | $0 \le \tau_1 \le 1$ ; $\tau_2 = 0$ | $\mathbf{C}_1$|
| **Regularized Canonical Correlation Analysis** [@Vinod1976; @Leurgans1993; @Shawe2004]           | $x$    | $0 \le \tau_1 \le 1$ ; $0 \le \tau_2 \le 1$ | $\mathbf{C}_1$|
...
| **PLS path modeling - mode B** [@Wold1982, @Tenenhaus2005]          | $|x|$  | $\tau_j = 0, j=1, \ldots, J$ | $c_{jk}=1$ for two connected block and $c_{jk} = 0$ otherwise |
...
| **Generalized CCA** [@Carroll1968b]                 | $x^2$  | $\tau_j=0, j=1, \ldots, J_1$ ; $\tau_j = 1, j=J_1+1, \ldots, J$ |  $\mathbf{C}_4$ |
| **Hierarchical PCA** [@Wold1996]                    | $x^4$  | $\tau_j = 1, j=1, \ldots, J$ ; $\tau_{J+1} = 0$ | $\mathbf{C}_4$ |
| **Multiple Co-Inertia Analysis** [@Chessel1996, @Westerhuis1998, @Smilde2003]     | $x^2$  | $\tau_j = 1, j=1, \ldots, J$ ; $\tau_{J+1} = 0$ | $\mathbf{C}_4$ |
| **Multiple Factor Analysis** [@Escofier1994]     | $x^2$  | $\tau_j = 1, j=1, \ldots, J+1$ | $\mathbf{C}_4$ |
...
[@Tenenhaus2011, @Tenenhaus2017] requires the handling of matrices of dimension 
...
implements these two formulations and selects automatically the best one.  
...
$\mathbf{y}_j= \mathbf{X}_j \mathbf{a}_j,j=1, \ldots,J$ are likely affected by 
the usefulness and relevance of the variables of each block. Accordingly, it is 
an important issue to identify within each block a subset of significant 
variables which are active in the relationships between blocks. SGCCA extends 
RGCCA to address this issue of variable selection  [@Tenenhaus2014b]. The SGCCA 
...
According to [@Witten2009a], solution of (\ref{update_SGCCA}) satisfies:
...
When such condition is fulfilled, the $\ell_2$-sphere of radius $1$ and the 
...
(POCS), also known as alternating projection method [@Boyd2003], can be 
used to determine the optimal $\lambda_j$ verifying the $\ell_1$-norm constraint. 
Here, a much faster approach described in [@Gloaguen2017] is implemented within 
the RGCCA package.
...
It implies that whatever the choice of the mode of delfation the block component
...
(`ade4:::mfa()/FactoMineR::MFA`). We follow the deflation 
strategy described in [@Chessel1996] (`ade4:::mcoa()`) for Multiple Co-inertia 
Analysis, which is one of the most popular and established methods of the 
multiblock literature.


Guidelines describing R/SGCCA in practice are provided in [@Garali2018].
...
In this section, we propose to reproduce some of the results presented in 
[@Tenenhaus2011] from the Russett data. The Russett dataset is available within 
the RGCCA package. The Russett data set [@Russett1964] are studied in [@Gifi1990]. 
...
the square root of its number of variables [@Westerhuis1998]. This two-step 
...
**Choice of the scheme function g**. Typical choices of scheme functions are $g(x) = x, x^2$, or $\vert x \vert$.  According to [@VandeGeer1984], a fair model is a model where all blocks contribute equally to the solution in opposition to a model dominated by only a few of the $J$ sets. If fairness is a major objective, the user must choose $m=1$. $m>1$ is preferable if the user wants to discriminate between blocks. In practice, $m$ is equal to $1$, $2$ or $4$. The higher the value of $m$ the more the method acts as block selector [@Tenenhaus2017].

RGCCA using the pre-defined design matrix $\mathbf{C}$, 
the factorial scheme ($g(x) = x^2$), $\tau = 1$ for all blocks (full covariance criterion) and a number of (orthogonal) components equal to $2$ for all blocks is obtained by specifying appropriately  the arguments `connection`, `scheme`, `tau`, `ncomp`, `comp_orth` in `rgcca()`. `verbose` (default value = `TRUE`) indicates that the progress will be reported while computing and that a plot representing the convergence of the algorithm will be returned.
...
```{r, fig.height = 12, fig.width=18, fig.cap = 'block weight vectors', fig.pos = "H"}
...
biplot (`type = "biplot"`).  This graphical displays allows visualizing the sources of variability within blocks, the relationships between variables within and between blocks and the amount of correlation between blocks. The graphical display of the countries obtained by crossing $\X_1 \ma a_1$ = Agricultural Inequality and $\X_2 \ma a_2$ = Industrial Development and marked with their political regime in 1960 is shown in below.
...
The Average Variance Explained (AVE) defined below is also reported in the axes 
of the Figure.  The AVE of block $\mathbf{X}_j$ for a specific block component 
...
AVE($\X_j$) varies between 0 and 1 and reflects the proportion of variance captured by $\mathbf{y}_j$.
...
```{r, fig.align='center', fig.cap = 'Average variance explained of the various blocks', fig.height = 12, fig.width=18, fig.pos = "H"}
...
```{r, fig.align='center', fig.cap = 'correlation circle associated with the two first components of Ariculture', fig.height = 12, fig.width=18, fig.pos = "H"}
...
```{r, fig.align='center', fig.cap = 'biplot associated with the two first components of Ariculture', fig.height = 12, fig.width=18, fig.pos = "H"}
...
```{r, fig.cap = 'boostrap confidence interval - component 1', fig.height = 12, fig.width=18, fig.pos = "H"}
...
In this section, we consider Multiple Co-Inertia Analysis  [@Chessel1996] (MCOA, 
also called MCIA in [@Cantini2021]) with $2$ components per block. 
...
Interestingly, this output reports the arguments that has been implicitly 
specified to reach MCOA. 
...
```{r, fig.align='center', fig.cap = 'A biplot graphical display of the countries obtained by crossing the two first components of the superblock. Individuals are labeled according to their political regime and variables according to their block membership.', fig.height = 12, fig.width=18, fig.pos = "H"}
...
available using the `print()/plot()` functions.
...
analytical formula [@Schafer2005] by setting the `tau` argument of the `rgcca()` 
function to `"optimal"`.
...
proposed in [@Witten2009a] has been also integrated within the RGCCA package 
through the `rgcca_permutation()` function. This function is used to select 
automatically the regularization parameters for R/SGCCA.

For each set of regularization parameters (generally this will be a 
$J$-dimensional vector), repeat the following `n_perm` times, for (`n_perm` 
large):
...
\end{enumerate}
Then choose the set of tuning parameters that yields the smallest value in step 
(\ref{p4}). This procedure is available though the `rgcca_permutation()` function.
...
By default, the `rgcca_permutation()` function takes 10 sets of tuning 
parameters between min values (0 for RGCCA and $1/sqrt(ncol)$ for SGCCA) and 1. 
Results of the permutation procedure are summarized/displayed using the generic 
`print()/plot()` function
...
be tested: from the minimum values (0 for RGCCA and $1/sqrt(ncol)$ for SGCCA) to 
...
`rgcca_cv()` functionand is used here for predicting the 
qualitative variable political regime from the two blocks 
`Agriculture inequality` and `Industrial development`.
...
The fitted cval object can be provided as output of `rgcca()` and 
...
```{r}
fit.rgcca = rgcca(blocks = blocks, response = 3, ncomp = 2, verbose = TRUE)
```
...
cross-validation using the `rgcca_cv()` function. The goal is to maximize the 
...
parameters for $X_1$ and $X_2$ to $0.2$, to achieve an attractive amount of 
sparsity. 
...
`"tau"`) within the `rgcca_permutation()` function.
...
path modeling to deal with missing data (see [@Tenenhaus2005]).
...
RGCCA for multigroup data [@Tenenhaus2014b] and RGCCA for multiway data 
[@Gloaguen2020, @Girka2023], RGCCA for (sparse and irregular) functional data [@Sort2023]. In addition, maximizing successive criteria may be 
seen as suboptimal from an optimization point of view where a single global 
criterion might be preferred. A global version of RGCCA [@Gloaguen2020b] 
which allows extracting simultaneously several components per block (no 
deflation procedure required) has been proposed. At last, it is possible to use 
RGCCA in structural equation modeling with latent and emergent variables for 
obtaining consistent and asymptotically normal estimators of the parameters [@Tenenhaus2023]. Work in progress includes the integration of all these novel 
